...
def convert_text(data_name, word_to_idx, ngram_size, dataset, context_size=5):
    ngram_features = []
    ngram_context = []
    ngram_lbls = []
    ...
    with open(data_name, "r") as f:
        for line in f:
            ...
            for i in range(padding, len(words)):
                ...
                ngram_features.append(ngram_prefix)
...
def convert_blanks(data_name, word_to_idx, ngram_size, dataset, context_size=5):
    ngram_features = []
    ngram_lbls = []
    ngram_queries = []
    ngram_context = []
...
def main(arguments):
    ...
    train_input, train_context, train_output = convert_text(train, word_to_idx, ngram_size, dataset, context_size)

    if valid:
        valid_input, valid_context, valid_output = convert_text(valid, word_to_idx, ngram_size, dataset, context_size)

    if valid_blanks:
        valid_blanks_input, valid_blanks_output, valid_blanks_queries, valid_blanks_context = convert_blanks(valid_blanks, word_to_idx, ngram_size, dataset, context_size)

    if test_blanks:
        test_blanks_input, _, test_blanks_queries, test_blanks_context = convert_blanks(test_blanks, word_to_idx, ngram_size, dataset, context_size)
