...
We use a slightly modified version of Kneser-Ney (discussed in stats.stackexchange.com/questions/114863/in-kneser-ney-smoothing-how-are-unseen-words-handled [put into bib]) to account for cases where a unigram is never found. Fix a ``discount parameter'' $0 < \delta < 1$. For unigrams, the equation is:
...
\textsc{Kneser-Ney (Bigrams)} & & & 111 & 3.86 \\
\textsc{Kneser-Ney (Trigrams)} & & & 188 & 4.52 \\
...
\begin{figure}
\centering
\subfloat[Bigrams]{ \includegraphics[scale=0.2]{laplace_2grams.png} \label{fig:laplace2} }
\hfill
\subfloat[Trigrams]{ \includegraphics[scale=0.2]{laplace_3grams.png} \label{fig:laplace3} }
...
We also implemented Kneser-Ney smoothing as an extension. We optimized the $\delta$ parameter via a linear search in intervals of $0.05$ from $0$ to $1$, graphed below. 
...
The count-based model results were very surprising. First of all, our Witten-Bell and Kneser-Ney bigram models had lower perplexity than the neural model on the valid set. Second of all, our trigram models universally performed much worse than our bigram models. 
